...
public class PlayerCharacter : MonoBehaviour, ICharacterController
{
    ...
    [Header("States")]
    [ShowOnly][SerializeField] private CharacterState _state;
    [ShowOnly][SerializeField] private CharacterState _lastState;
    [Space]
    ...
    [ShowOnly][SerializeField] private float currentStamina;
    [ShowOnly][SerializeField] private bool isExhausted;
    [Space]
    ...
    // private variables
    [HideInInspector] public bool playerTrapped = false;
    ...
    public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        if(motor.GroundingStatus.IsStableOnGround)
        {
            ...
            else
            {
                ...
                if (isExhausted)
                {
                    if (currentStamina >= maxStamina) // Only recover from exhaustion at full stamina
                    {
                        isExhausted = false;
                    }
                }
            ...
            if(_state.Stance is Stance.Stand or Stance.Crouch or Stance.Sprint)
            {
                ...
                if(currentVelocity != Vector3.zero)
                {
                    ...
                    {
                        NoiseManager.WalkNoise(transform.position);
                    }
                    else if(_state.Stance is Stance.Sprint && speed == sprintSpeed)
                    {
                        NoiseManager.sprintNoise(transform.position);
                    }
                }
    ...
    public Vector3 GetCameraTargetLocalPosition() => cameraTarget.localPosition;
    public Transform GetFlashlightTarget() => flashlightTarget;
